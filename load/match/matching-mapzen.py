...
cur.execute('truncate table gtfs.matched_ways');
cur.execute('truncate table gtfs.matched_point');
cur.execute('truncate table gtfs.matched_shape');
pg.commit()
...
for i in range(len(shapes)):
	...
	coord_list = [ {'lon':latlon[0], 'lat':latlon[1]} for latlon in coords]
	...
	matched_shape['shape_seq_num'] = range(0, len(matched_shape))
