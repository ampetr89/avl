...
create table osm.road_segments 
    AS
select way_id, ST_x(the_geom) as lon, ST_y(the_geom) as lat, path[1] - 1 as line_num, path[2] - 1 as point_num
from
...
create index idx_road_segments__geom on osm.road_segments(way_id);
create index idx_road_segments__geom on osm.road_segments(lon, lat)
;
----
...
select shape_id, edge_seq_num, a.way_id, 
  ...
  row_number() over (partition by shape_id, edge_seq_num order by abs(a.start_lat - b2.lat) + abs(a.start_lon - b2.lon)) as r2
from etl.matched_ways_endpoints as a
left join osm.road_segments as b1
 on a.way_id = b1.way_id 
 and abs(a.start_lat - b1.lat) < 0.0001
 and abs(a.start_lon - b1.lon) < 0.0001 
left join osm.road_segments as b2
